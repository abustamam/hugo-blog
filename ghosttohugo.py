...
def migrate(filepath, hugodir):
    ...
    for post in data['posts']:
        ...
        ts = gt(post["created_at"]) 
        print(ts)
        print(post) 
        
        banner = "" if post["feature_image"] is None else post["feature_image"]
        ...
        hugopost = _post.format(markdown=json.loads(post["mobiledoc"])["cards"][0][1]["markdown"],
                                title=post["title"],
                                draft=draft,
                                slug=post["slug"],
                                date=ts.isoformat(),
